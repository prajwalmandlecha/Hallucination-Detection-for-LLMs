(function attachDeepSeekAttachmentObserver() {
  if (window.__hdDeepSeekAttachmentObserverAttached) return;
  window.__hdDeepSeekAttachmentObserverAttached = true;

  const CONTENT_SCRIPT_BUILD_TAG = "deepseek-attachment-observer-v1";
  ...
  function getConversationId() { return window.location.pathname.match(/\/chat\/([^/?#]+)/)?.[1] || null; }
  function getConversationTitle() { return normalizeText(document.title.replace(/\s*[-–|]\s*DeepSeek\s*$/i, "")) || null; }
  function getConversationContext() { return { platform: "deepseek", id: getConversationId(), url: window.location.href, title: getConversationTitle() }; }
  ...
  async function pingBackgroundServiceWorker() {
    ...
    console.log("[DeepSeek Extractor] Content script booted:", { build: CONTENT_SCRIPT_BUILD_TAG, background: response });
  }
  ...
  async function captureFiles(fileLikeList, source) {
    ...
    for (const file of files) {
      ...
      console.log("[DeepSeek Extractor] Captured DeepSeek attachment:", payload);
