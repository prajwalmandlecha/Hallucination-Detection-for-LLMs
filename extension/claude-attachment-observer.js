(function attachClaudeAttachmentObserver() {
  if (window.__hdClaudeAttachmentObserverAttached) {
    return;
  }

  window.__hdClaudeAttachmentObserverAttached = true;
  const CONTENT_SCRIPT_BUILD_TAG = "claude-attachment-observer-v1";
  ...
  function getConversationTitle() {
    const rawTitle = document.title.replace(/\s*[\\|]\s*Claude\s*$/i, "");
  ...
  function shouldCaptureFile(file, conversationId) {
    ...
    if (recentCaptureKeys.has(key)) return false;
      ...
      try {
        chrome.runtime.sendMessage(message, (response) => {
          if (chrome.runtime.lastError) {
            resolve({ __runtimeError: chrome.runtime.lastError.message || "Unknown runtime error" });
            return;
          }
          resolve(response || null);
        });
      } catch {
        resolve({ __runtimeError: "Failed to call chrome.runtime.sendMessage" });
      }
    });
  }
  ...
  async function pingBackgroundServiceWorker() {
    ...
    console.log("[Claude Extractor] Content script booted:", {
  ...
  function isTextLikeFile(file) {
    ...
    return [".txt", ".md", ".json", ".csv", ".tsv", ".js", ".ts", ".jsx", ".tsx", ".html", ".css", ".xml", ".yml", ".yaml"]
  ...
  async function buildDebugPreview(file) {
    try {
      if (isTextLikeFile(file)) {
        ...
        return { kind: "text", characterCount: text.length, preview: normalizeText(text).slice(0, 2000) || "" };
      }
      const buffer = await file.arrayBuffer();
      const bytes = new Uint8Array(buffer);
      return { kind: "binary", byteCount: bytes.length, previewHex: toHexPreview(bytes, 32) };
    } catch (error) {
  ...
  async function forwardAttachmentToBackend(file, metadata) {
    try {
      ...
      const response = await sendRuntimeMessage({
        ...
        payload: { metadata, fileData }
      });
      if (response?.__runtimeError) {
        return { attempted: true, ok: false, status: "extension_error", error: response.__runtimeError };
      }
      if (!response) {
        return { attempted: true, ok: false, status: "extension_error", error: "No response from background." };
      }
      return response;
    } catch (error) {
      return { attempted: true, ok: false, status: "serialization_error", error: String(error) };
    }
  }
  ...
  async function captureFiles(fileLikeList, source) {
    ...
    for (const file of files) {
      ...
      console.log("[Claude Extractor] Captured Claude attachment:", payload);
