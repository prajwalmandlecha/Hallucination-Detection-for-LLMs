(function attachGeminiAttachmentObserver() {
  if (window.__hdGeminiAttachmentObserverAttached) {
    return;
  }

  window.__hdGeminiAttachmentObserverAttached = true;
  const CONTENT_SCRIPT_BUILD_TAG = "gemini-attachment-observer-v1";
  ...
  function getConversationTitle() {
    const rawTitle = document.title.replace(/\s*-\s*Gemini\s*$/i, "");
  ...
  async function pingBackgroundServiceWorker() {
    ...
    console.log("[Gemini Extractor] Content script booted:", {
  ...
  async function captureFiles(fileLikeList, source) {
    ...
    for (const file of files) {
      ...
      console.log("[Gemini Extractor] Captured Gemini attachment:", payload);
